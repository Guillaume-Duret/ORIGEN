...
class DINOv2_MLP(nn.Module):
    def __init__(self, 
        ...
        if dino_mode == 'base':
            ...
            self.dinov2 = FLIP_DINOv2.from_pretrained(DINO_GIANT, torch_dtype=dtype)
        print(dino_mode)
    ...
    def inference(self, inputs):
        dino_outputs = self.dinov2(inputs)
        dino_seq = dino_outputs.last_hidden_state
        dino_seq = dino_seq[:,0,:]
        down_sample_out = self.down_sampler(dino_seq)
        down_sample_out[:, 0:360] = F.softmax(down_sample_out[:, 0:360], dim=-1) # azimuth
        down_sample_out[:, 360:540] = F.softmax(down_sample_out[:, 360:540], dim=-1)  # polar
        down_sample_out[:, 540:720] = F.softmax(down_sample_out[:, 540:720], dim=-1)  # rotation 
        return down_sample_out
